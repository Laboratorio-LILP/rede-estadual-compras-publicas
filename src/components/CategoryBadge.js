...
// Normaliza o nome da categoria para casar com o mapa de ícones
...
// Ícone por categoria. Traçado (stroke) usa currentColor, então a cor
// é herdada do selo.
...
// Ícone padrão (tag) para categorias sem mapeamento próprio.
const FALLBACK_ICON = (
  <path strokeLinecap="round" strokeLinejoin="round" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
);

export function getCategoryIcon(name) {
  return CATEGORY_ICONS[normalize(name)] || FALLBACK_ICON;
}

export default function CategoryBadge({ id, name, color, className = '' }) {
  const content = (
    <>
      <svg
  ...
  return (
    <Link to={`/category/${id}`} className={`${classes} transition hover:opacity-90`} style={{ backgroundColor: color }}>
      {content}
    </Link>
  );
}
