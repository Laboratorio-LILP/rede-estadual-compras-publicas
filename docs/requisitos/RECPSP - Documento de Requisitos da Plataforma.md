...
| Campo | Valor |
|---|---|
| Versão | 1.0 — minuta para validação da equipe |
| Data | 26/08/2026 |
...
| RNF-SEG-03 | Segredos somente por variável de ambiente, sem fallback; nenhuma credencial de seed em código; `JWT_SECRET` obrigatório | M | Compose já falha sem `JWT_SECRET`; seed ainda cria `admin`/usuários de teste com senha em código | Padrão LILP (repo-modelo BDLP); CLAUDE.md da frente |
| RNF-SEG-04 | CSP estrita em produção; CORS por ambiente (`ALLOWED_ORIGINS`); rate limit (hoje só na autenticação — estender ao restante da API) | M | CSP desativada; CORS por env resolvido em 26/08 | CLAUDE.md da frente |
...
| RNF-SEG-06 | Moderação prévia permanente de todo conteúdo público (mitigação de risco político e de qualidade) | M | Implementada para tópicos | Apresentação 26/08 (Laís); Regimento (moderação ativa) |
